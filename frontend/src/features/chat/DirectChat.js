...
import '../../components/RoomChat.css'; // Import shared styles
...
const DirectChat = ({ user, friendNickname }) => {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      const data = await apiGet(`/users/chat/direct/${encodeURIComponent(friendNickname)}/`);
      setMessages(Array.isArray(data) ? data : (data.results || []));
    } catch (err) {
      console.error("1:1 채팅 불러오기 실패:", err);
    }
  }, [friendNickname]); // user.nickname dependency removed as it's not used in URL
  ...
  const handleSend = async (msgText = null) => {
    ...
    if (!msgText && showPlusMenu) setShowPlusMenu(false);
    ...
    try {
      await apiPostForm(`/users/chat/direct/`, formData);
      if (!msgText) setInput("");
    ...
    <MobileLayout className="chat-layout">
      ...
      <div className="room-chat-page">
        ...
        <div className="rc-header">
          <button className="rc-back-btn" onClick={() => window.history.back()}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M15 18l-6-6 6-6" />
            </svg>
          </button>
          <div className="rc-header-title-box">
            {/* Friend Profile Image Placeholder */}
            <img src="https://placehold.co/44x44" alt={friendNickname} className="rc-room-img" />
                    ...
                    <div className="rc-bubble-row">
                      ...
                      {isVote && voteData ? (
                        ...
                        <div className="rc-bubble">
                          {msg.message}
                          {msg.image_url && <img src={`${API_BASE_SERVER}${msg.image_url}`} alt="첨부 이미지" style={{ maxWidth: '100%', borderRadius: '10px', marginTop: '5px' }} />}
                        </div>
                      )}
                      <div className="rc-msg-info">
                        {(msg.sender === user.nickname && !msg.is_read) && <div className="rc-unread-count">1</div>}
        ...
        <div className="rc-input-area">
          ...
          <button type="button" className="rc-input-plus-btn" onClick={togglePlusMenu} style={{ border: '1.5px solid #64748B', background: 'transparent' }}>
