...
//  (error) => Promise.reject(error)
//);
// [전체 코드] src/api/api.js (70단계 + 401 자동 갱신 로직 추가)
import axios from "axios";
...
// --- 👇👇👇 [수정] 4-1. 응답 인터셉터 (401 에러 처리) 주석 해제 및 수정 ---
api.interceptors.response.use(
  (response) => response,
  async (error) => {
    const originalRequest = error.config;
    ...
    // 401 에러이고, 재시도하지 않은 요청인 경우
    // [!] 토큰 갱신 요청 자체에서 401이 발생한 경우는 무한 루프를 방지하기 위해 제외
    if (error.response?.status === 401 && !originalRequest._retry && !originalRequest.url.includes('/users/token/refresh/')) {
      originalRequest._retry = true;
      ...
      if (refreshToken) {
        try {
          ...
          // [!] 갱신 요청은 인터셉터가 없는 순수 axios 사용 또는 adminApi 사용
          // 여기서는 adminApi를 사용 (baseURL이 /api/v1로 동일하므로)
          const response = await adminApi.post(
            `/users/token/refresh/`, // [!] Django의 리프레시 URL
            { refresh: refreshToken }
          );
