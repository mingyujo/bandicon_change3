...
# --- 👇 [신규] 팝업 공지 관리 Views ---
from support_app.models import PopupAnnouncement
from support_app.serializers import PopupAnnouncementSerializer, PopupAnnouncementCreateSerializer

class AdminPopupAnnouncementListView(views.APIView):
    """
    GET /api/v1/admin/popup-announcements
    POST /api/v1/admin/popup-announcements
    """
    permission_classes = [IsOperator]

    def get(self, request):
        # 최신순 정렬
        announcements = PopupAnnouncement.objects.all().order_by('-created_at')
        serializer = PopupAnnouncementSerializer(announcements, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)

    def post(self, request):
        serializer = PopupAnnouncementCreateSerializer(data=request.data)
        if serializer.is_valid():
            # 작성자는 현재 로그인한 운영자 (request.user)
            serializer.save(created_by=request.user)
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

class AdminPopupAnnouncementDeactivateView(views.APIView):
    """
    PUT /api/v1/admin/popup-announcements/<pk>/deactivate
    """
    permission_classes = [IsOperator]

    def put(self, request, pk):
        announcement = get_object_or_404(PopupAnnouncement, pk=pk)
        announcement.is_active = False
        announcement.save()
        return Response({"detail": "공지가 비활성화되었습니다."}, status=status.HTTP_200_OK)
