...
class ClanSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'name', 'description', 'owner', 'created_at', 'image', 'youtube_url',
...
class ClanDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'name', 'description', 'created_at', 'owner', 'admins', 'members', 
                  'image', 'youtube_url', 'announcements', 'events', 'boards', 'join_requests', 'status')
...
class ClanChatSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'clan', 'sender', 'sender_nickname', 'message', 'timestamp', 'unread_count']
        # ▲▲▲ [수정 5] ▲▲▲
        read_only_fields = ['clan', 'sender']

    def get_unread_count(self, obj):
        # 전체 멤버 수 - 읽은 사람 수 (단, 보낸 사람은 제외하거나 포함? 보통 포함해서 계산하되, 읽은 목록에 sender도 들어가는지 확인)
        # Assuming sender usually reads their own message immediately or implicitly.
        # But 'read_by' logic needs to be consistent. 
        # For efficiency, we can use: obj.clan.members.count() - obj.read_by.count()
        # Note: This might be N+1 query if not optimized.
        # Ideally, annotate in View. But for now, simple implementation.
        total_members = obj.clan.members.count()
        read_count = obj.read_by.count()
        count = total_members - read_count
