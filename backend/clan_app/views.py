...
class ClanListCreateAPIView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        # DEBUG: Check if youtube_url is present
        print(f"DEBUG: Clan Create Validated Data: {serializer.validated_data}")
        # 1. 클랜 생성 (status='pending'은 모델 디폴트)
        clan = serializer.save(owner=self.request.user)
        
        # [수정] 생성자(클랜장)는 자동으로 멤버 및 운영진에 추가
        clan.members.add(self.request.user)
        clan.admins.add(self.request.user)
    ...
    def post(self, request, pk):
        ...
        try:
            Alert.objects.create(
                ...
                related_url=f"/clans/{clan.id}/" # 클릭 시 클랜 상세 페이지로 이동
            )
        except Exception as e:
...
class ClanJoinRequestActionView(APIView):
    ...
    """
    permission_classes = [IsClanOwnerOrAdmin] # 클랜장 또는 관리자 가능
...
class ClanTransferOwnershipView(APIView):
    """
    (POST) /api/v1/clans/<int:clan_id>/transfer-ownership/
    클랜장 위임 (클랜장 -> 간부)
    """
    permission_classes = [IsClanOwner]
    ...
    def post(self, request, clan_id):
        ...
        if not target_user_id:
            return Response({"detail": "위임할 할 대상(target_user_id)이 필요합니다."}, status=status.HTTP_400_BAD_REQUEST)

        target_user = get_object_or_404(User, pk=target_user_id)

        # 대상이 간부인지 확인 (클랜장 위임은 간부에게만 가능하도록 제약)
        if not clan.admins.filter(id=target_user.id).exists():
             return Response({"detail": "클랜장 위임은 운영진(간부)에게만 가능합니다."}, status=status.HTTP_400_BAD_REQUEST)

        # 1. 기존 클랜장 -> 간부로 변경 (선택사항, 보통 조력자로 남음)
        # 1-1. 기존 클랜장을 admins에 추가
        clan.admins.add(clan.owner)
        
        # 2. 새 클랜장 설정
        clan.owner = target_user
        
        # 3. 새 클랜장은 admins 목록에서 제거 (Owner는 Admin 목록에 중복해서 들어가지 않음. 모델링에 따라 다름)
        # 로직: admins는 '추가 관리자' 개념이므로, owner는 admins에서 빠지는게 깔끔함.
        if clan.admins.filter(id=target_user.id).exists():
            clan.admins.remove(target_user)
        ...
        try:
            Alert.objects.create(
                user=target_user,
                message=f"'{clan.name}' 클랜장이 되셨습니다!",
                related_url=f"/clans/{clan.id}/"
            )
            Alert.objects.create(
                user=request.user, # 구 클랜장
                message=f"'{clan.name}' 클랜장을 '{target_user.nickname}'님에게 위임했습니다.",
                related_url=f"/clans/{clan.id}/"
            )
        except Exception as e:
            print(f"Error creating transfer alerts: {e}")

        return Response({"detail": f"클랜장을 {target_user.nickname}님에게 위임했습니다."}, status=status.HTTP_200_OK)
...
class ClanApproveAllView(APIView):
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
            return Response({"detail": f"서버 에러 발생: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class ClanRoomListAPIView(generics.ListCreateAPIView): # [수정] ListCreateAPIView로 변경
    ...
    def create(self, request, *args, **kwargs):
        ...
        # [추가] 거절된 클랜은 방 생성 불가
        if clan.status == 'rejected':
            return Response({"detail": "거절된 클랜에서는 방을 생성할 수 없습니다."}, status=status.HTTP_400_BAD_REQUEST)
