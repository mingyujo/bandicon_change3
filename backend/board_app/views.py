...
class PostDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def perform_update(self, serializer):
        # [수정] 작성자 또는 운영자만 수정 가능
        if serializer.instance.author != self.request.user and self.request.user.role != 'OPERATOR':
             raise permissions.PermissionDenied("게시글 작성자 또는 운영자만 수정할 수 있습니다.")
        serializer.save()

    def perform_destroy(self, instance):
        # [수정] 작성자 또는 운영자만 삭제 가능
        if instance.author != self.request.user and self.request.user.role != 'OPERATOR':
            raise permissions.PermissionDenied("게시글 작성자 또는 운영자만 삭제할 수 있습니다.")
        instance.delete()
...
class CommentDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def perform_update(self, serializer):
        # [수정] 작성자 또는 운영자만 수정 가능
        if serializer.instance.author != self.request.user and self.request.user.role != 'OPERATOR':
            raise permissions.PermissionDenied("댓글 작성자 또는 운영자만 수정할 수 있습니다.")
        serializer.save()

    def perform_destroy(self, instance):
        # [수정] 작성자 또는 운영자만 삭제 가능
        if instance.author != self.request.user and self.request.user.role != 'OPERATOR':
            raise permissions.PermissionDenied("댓글 작성자 또는 운영자만 삭제할 수 있습니다.")
        instance.delete()

class CommentToggleLikeView(views.APIView):
    permission_classes = [permissions.IsAuthenticated]
...
class ClanHotPostListView(generics.ListAPIView):
    """
    클랜의 인기 게시글 (좋아요 순) 조회
    GET /api/v1/boards/clan/<int:clan_id>/hot-posts/
    """
    serializer_class = PostListSerializer
    permission_classes = [permissions.IsAuthenticated]
    ...
    def get_queryset(self):
        ...
        return Post.objects.filter(clan_board__clan_id=clan_id) \
