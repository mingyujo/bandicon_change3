...
class CommentSerializer(serializers.ModelSerializer):
    ...
    post_author = serializers.ReadOnlyField(source='post.author.nickname')
    ...
    class Meta:
        model = Comment
        fields = ['id', 'post', 'post_id', 'post_title', 'post_author', 'post_likes', 'post_comments', 'author', 'content', 'created_at', 'updated_at', 'is_anonymous', 'likes_count', 'replies_count', 'is_liked', 'is_writer', 'parent']
        read_only_fields = ['author', 'post_id', 'post_title', 'post_author', 'post_likes', 'post_comments', 'post', 'likes_count', 'replies_count', 'is_liked', 'is_writer']
        # extra_kwargs = {
        #     'post': {'required': True},
        # }
    ...
    def get_is_writer(self, obj):
        # 작성자인지 여부 (글 작성자? 아니면 댓글 작성자? "is_writer" usually means "is this comment written by the current user" OR "is this comment written by the POST author"?)
        # UI shows "익명 (작성자)". In many Korean apps "작성자" tag on a comment means "This comment was written by the *Post Author*".
        # Let's assume it means "Post Author".
        # But wait, `comment.is_writer` in local usually means "Is this MY comment".
        # The UI image showed "익명 (작성자)". This typically labels the OP (Original Poster) in the comments section.
        # So I will implement `is_post_author`.
        # Check `PostDetail.js`: `comment.is_writer` was used to show "(작성자)" label.
        # Code: `{comment.is_writer && ... (작성자)}`. 
        # So yes, it means "Is Post Author". 
        return obj.author == obj.post.author
