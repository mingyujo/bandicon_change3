...
class SessionSerializer(serializers.ModelSerializer):
    ...
    # [추가] 프로필 이미지 URL
    participant_profile_image = serializers.SerializerMethodField()

    class Meta:
        model = Session
        fields = ['id', 'session_name', 'participant_nickname', 'reservations', 'participant_profile_image']

    def get_participant_profile_image(self, obj):
        if not obj.participant_nickname:
            return None
        user = User.objects.filter(nickname=obj.participant_nickname).first()
...
class GroupChatSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = GroupChat
        fields = ['id', 'sender', 'message', 'timestamp', 'unread_count', 'image_url']
        read_only_fields = ['id', 'sender', 'timestamp', 'unread_count'] # sender는 view에서 채움
    
    unread_count = serializers.SerializerMethodField()

    def get_unread_count(self, obj):
        participants = set()
        participants.add(obj.room.manager_nickname)
        for s in obj.room.sessions.all():
            if s.participant_nickname:
                participants.add(s.participant_nickname)
        
        total = len(participants)
        read = obj.read_by.count()
