...
class RoomListCreateAPIView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        required_session = self.request.query_params.get('required_session')
        search_query = self.request.query_params.get('search')

        if search_query:
            queryset = queryset.filter(
                Q(title__icontains=search_query) |
                Q(song__icontains=search_query) |
                Q(artist__icontains=search_query)
            )
        
        # 2. Annotation 준비 (빈 세션 수)
        # 빈 세션 정의: participant_nickname이 NULL 이거나 빈 문자열
        empty_session_condition = Q(sessions__participant_nickname__isnull=True) | Q(sessions__participant_nickname='')
        
        queryset = queryset.annotate(
            empty_count=Count('sessions', filter=empty_session_condition)
        )

        ordering = []

        # 3. 특정 세션 필터 (정렬 우선순위 적용)
        # "해당 세션이 차있지 않은 방 순서대로 정렬"
        if required_session:
            # 해당 세션 이름이 확인되는(포함되는) + 비어있는 경우의 수
            # (예: '기타' 검색 시 '리드기타', '일렉기타' 등도 포함)
            target_empty_condition = Q(sessions__session_name__icontains=required_session) & empty_session_condition
            queryset = queryset.annotate(
                target_empty_count=Count('sessions', filter=target_empty_condition)
            )
            # 타겟 세션이 비어있는 방이 먼저 오도록 내림차순 정렬
            ordering.append('-target_empty_count')
        
        # 4. 일반 정렬 기준 적용 (2순위)
        if sort_by == 'empty_desc':     # 빈 세션 많은 순
            ordering.append('-empty_count')
            ordering.append('-created_at')
        elif sort_by == 'empty_asc':    # 빈 세션 적은 순
            ordering.append('empty_count')
            ordering.append('-created_at')
        elif sort_by == 'oldest':       # 오래된 순
            ordering.append('created_at')
        else:                           # 최신순 (default)
            ordering.append('-created_at')
...
class MyRoomListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        # 내가 매니저이거나, 내가 세션에 참여 중이거나, 내가 세션에 예약(신청) 중인 방
        ...
        ).prefetch_related('sessions').distinct().order_by('-last_activity')
...
class RoomConfirmView(APIView):
    ...
    def post(self, request, pk):
        ...
        if not (is_manager or is_clan_admin):
            raise PermissionDenied("합주 확정 권한이 없습니다.")
...
class RoomEndView(APIView):
    ...
    def post(self, request, pk):
        ...
        if not (is_manager or is_clan_admin):
            raise PermissionDenied("합주 종료 권한이 없습니다.")
        ...
        for session in room.sessions.all():
            if session.participant_nickname:
                participant_nicknames.add(session.participant_nickname)
...
class VerifyRoomPasswordView(APIView):
    """
    (POST) /api/v1/rooms/<int:pk>/verify_password/
    비밀방 비밀번호 검증
    """
    permission_classes = [permissions.IsAuthenticated]
    ...
    def post(self, request, pk):
        ...
        print(f"DEBUG: Room[{room.id}] stored='{room.password}', input='{password}'")
        if not room.is_private:
             return Response({"detail": "비밀방이 아닙니다."}, status=status.HTTP_400_BAD_REQUEST)
        
        if str(room.password).strip() == str(password).strip():
            return Response({"detail": "승인되었습니다."}, status=status.HTTP_200_OK)
        else:
            return Response({"detail": "비밀번호가 다릅니다."}, status=status.HTTP_400_BAD_REQUEST)

class RoomImageUploadView(APIView):
    """
    (POST) /api/v1/rooms/<int:pk>/image/
    방 대표 이미지 업로드 (방장 또는 클랜 관리자)
    """
    permission_classes = [permissions.IsAuthenticated]
    ...
    def post(self, request, pk):
        ...
        if not (is_manager or is_clan_admin):
            raise PermissionDenied("방 이미지 변경 권한이 없습니다.")

        image_file = request.FILES.get('image')
        if not image_file:
            return Response({"detail": "이미지 파일이 제공되지 않았습니다."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        # 변경된 이미지 URL 반환
        return Response({"image_url": room.image.url, "detail": "방 이미지가 변경되었습니다."}, status=status.HTTP_200_OK)
...
class MannerEvaluationAPIView(APIView):
    ...
    def post(self, request, pk):
        ...
        # [수정] 평가 완료 시 해당 방의 평가 요청 알림을 읽음 처리
        ...
        # [수정] 피평가자들의 매너 점수 업데이트 (평균 계산)
        from django.db.models import Avg
        for target_user in set([e.target for e in evaluations_to_create]):
            # 해당 유저가 받은 모든 평가의 평균 점수 계산
            # [디버그] 평가 수와 평균 확인
            result = Evaluation.objects.filter(target=target_user).aggregate(Avg('score'))
            avg_score = result['score__avg']
            
            if avg_score is not None:
                # 반올림하여 정항 처리 (int()는 내림이므로 round 사용 고려)
                new_score = int(round(avg_score))
                target_user.score = new_score
                # [중요] 다른 필드(닉네임 등) 덮어쓰기 방지를 위해 update_fields 사용
                target_user.save(update_fields=['score'])
                print(f"[Evaluation] User {target_user.nickname} score updated to {new_score} (Avg: {avg_score})")
...
class GroupChatView(generics.ListCreateAPIView):
    ...
    def list(self, request, *args, **kwargs):
        # [수정] 조회 시 자동으로 읽음 처리 (Side Effect) - 먼저 처리해야 응답에 반영됨
        try:
            user = request.user
            if user.is_authenticated:
                room_id = self.kwargs.get('room_id')
                from .models import GroupChat # Local import
                
                # DB에서 '이 방의 안 읽은 메시지'를 찾아서 처리
                unread_objs = GroupChat.objects.filter(room_id=room_id).exclude(read_by=user)
                if unread_objs.exists():
                    for chat in unread_objs:
                        chat.read_by.add(user)
        except Exception as e:
            print(f"Error marking read in GroupChatView: {e}")
            pass
    ...
    def get_queryset(self):
        ...
        #    f.write(f"[GroupChatView] fetching chat for room {room_id}\n")

        return GroupChat.objects.filter(room_id=room_id).select_related('room').prefetch_related('room__sessions', 'read_by').order_by('timestamp')
